...
stark::RigidBodyDynamics::RigidBodyDynamics(stark::core::Stark& stark)
{
	...
	// The rigid body dynamics callback shall be one of the first to be executed, since it resets the forces, torques, accelerations and angular accelerations
	stark.callbacks.before_time_step.insert(stark.callbacks.before_time_step.begin(), [&]() { this->_before_time_step(stark); });
...
int stark::RigidBodyDynamics::add(const std::string& label)
{
	...
	this->delta_t.push_back(Eigen::Vector3d::Zero());
...
{
	// Reset of forces, torques, accelerations & angular accelerations
	std::fill(this->force.begin(), this->force.end(), Eigen::Vector3d::Zero());
	std::fill(this->a.begin(), this->a.end(), Eigen::Vector3d::Zero());
	std::fill(this->torque.begin(), this->torque.end(), Eigen::Vector3d::Zero());
	std::fill(this->aa.begin(), this->aa.end(), Eigen::Vector3d::Zero());
	...
	std::fill(this->delta_t.begin(), this->delta_t.end(), Eigen::Vector3d::Zero());
}
void stark::RigidBodyDynamics::_on_time_step_accepted(stark::core::Stark& stark)
{
	...
	for (int i = 0; i < this->get_n_bodies(); i++) {
		...
		this->delta_t[i] = this->t1[i] - this->t0[i];
	}
