 ...
 *  Modified by Lukas Westhofen / BjoernBinaer
 */
...
std::vector<stark::Mesh<3>> stark::load_obj(const std::string& path)
{
	...
	else if (total_n_vertices > 0)
	{
		Mesh<3> tri_mesh;

		for (unsigned int i = 0; i < total_n_vertices; i++)
		{
			tinyobj::real_t vx = attrib.vertices[(int) (3 * i + 0)];
			tinyobj::real_t vy = attrib.vertices[(int) (3 * i + 1)];
			tinyobj::real_t vz = attrib.vertices[(int) (3 * i + 2)];
...
void stark::write_VTK(const std::string &path, const std::vector<Eigen::Vector3d> &vertices, const std::vector<Eigen::Vector3d>& directions)
{
	vtkio::VTKFile vtk_file;
	if (vertices.size() == 0) {
		vtk_file.write_empty(path);
		...
		vtk_file.set_points_from_twice_indexable(vertices);
		vtk_file.set_cells_as_particles(vertices.size());
		vtk_file.set_point_data_from_twice_indexable("orientations", directions, vtkio::AttributeType::Vectors);
		vtk_file.write(path);
	}
}
...
void stark::sampleTriangleMesh(std::vector<Eigen::Vector3d> &vertices, std::vector<std::array<int, 3>> triangles,
									  const double radius, const std::array<unsigned int, 3>& resolution,
									  std::vector<Eigen::Vector3d> &sample_positions)
{
	...
	for (const auto& tri : triangles)
		tris.push_back({
						   static_cast<unsigned int>(tri[0]),
						   static_cast<unsigned int>(tri[1]),
						   static_cast<unsigned int>(tri[2])
					   });
	Discregrid::TriangleMesh triMesh(vertices, tris);
	Discregrid::MeshDistance md(triMesh);
	Eigen::AlignedBox3d domain;
	domain.extend(box.min().cast<double>());
	domain.extend(box.max().cast<double>());
	...
	const double diameter = static_cast<double>(2.0) * radius;

	// sample object
	const unsigned int numberOfSamplePoints = (((unsigned int)((1.0f / diameter) * (box.max()[2] - box.min()[2]))) + 1) *
											  (((unsigned int)((1.0f / diameter) * (box.max()[1] - box.min()[1]))) + 1) *
											  (((unsigned int)((1.0f / diameter) * (box.max()[0] - box.min()[0]))) + 1);

	unsigned int currentSample = 0;
	int counter_x = 0;
	int counter_y = 0;
	double xshift = diameter;
	double yshift = diameter;

	for (double z = box.min()[2]; z <= box.max()[2]; z += diameter)
	{
		for (double y = box.min()[1]; y <= box.max()[1]; y += yshift)
		{
			for (double x = box.min()[0]; x <= box.max()[0]; x += xshift)
			{
				Eigen::Vector3d particlePosition;
				// Use center of voxel
				particlePosition = Eigen::Vector3d(x + radius, y + radius, z + radius);

				const double dist = distanceField->interpolate(0, particlePosition);

				if (dist < 0.0)
					sample_positions.push_back(particlePosition);
				currentSample++;
				counter_x++;
			}
			counter_x = 0;
