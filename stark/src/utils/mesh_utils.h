...
namespace stark
{
	...
	void write_VTK(const std::string &path, const std::vector<Eigen::Vector3d> &vertices, const std::vector<Eigen::Vector3d>& directions);
	...
	// Sampling
	void sampleTriangleMesh(std::vector<Eigen::Vector3d>& vertices, std::vector<std::array<int, 3>> triangles, const double radius, const std::array<unsigned int, 3>& resolution, std::vector<Eigen::Vector3d>& sample_positions);
